...
t("ow.maps exist", !!MAPS.route_a && !!MAPS.cave_a);
...
const flatO = flatMap("route_a"), flatC = flatMap("cave_a");
for (const ch of [",", ";", "~", "o", "C", "D", "H"]) t("ow.outdoor has tile " + ch, flatO.includes(ch));
for (const ch of ["~", "o", "X", "B"]) t("ow.cave has tile " + ch, flatC.includes(ch));
...
t("ow.tiles legend covers maps", flatO.concat(flatC).split("").every((ch) => !!TILES[ch]));
